...
public struct SlideMenuOptions {
    ...
    public static var rightViewWidth: CGFloat = 350
...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    deinit { }
    open func initView() {
        SlideMenuOptions.rightViewWidth = UIDevice.isPad ? 380 : 315             // ADDED TO CHANGE WIDTH OF HAMBURGER MENU
...
//            addLeftGestures()             COMMENTED TO REMOVE LEFT PANGESTURE
        }
...
//            if self.leftPanGesture != nil && self.leftPanGesture != nil {          COMMENTED TO REMOVE LEFT PANGESTURE
...
//            }
    ...
    open func addRightGestures() {
        if rightViewController != nil {
//            if SlideMenuOptions.panGesturesEnabled {             COMMENTED TO REMOVE RIGHT PANGESTURE ONLY
...
//            }
    ...
    //          COMMENTED RIGHT PAN GESTURE HANDLING
...
//    }
    open func openLeftWithVelocity(_ velocity: CGFloat) {
        ...
        addShadowToView(leftContainerView)
        UIView.animate(withDuration: duration, delay: 0.0, options: SlideMenuOptions.animationOptions) {
    ...
    fileprivate func applyLeftContentViewScale() {
        let openedLeftRatio: CGFloat = getOpenedLeftRatio()
        _ = 1.0 - ((1.0 - SlideMenuOptions.contentViewScale) * openedLeftRatio)
        _ = SlideMenuOptions.leftViewWidth + leftContainerView.frame.origin.x
    }
    fileprivate func applyRightContentViewScale() {
        let openedRightRatio: CGFloat = getOpenedRightRatio()
        _ = 1.0 - ((1.0 - SlideMenuOptions.contentViewScale) * openedRightRatio)
        _ = rightContainerView.frame.origin.x - mainContainerView.frame.size.width
    }
    ...
    fileprivate func removeViewController(_ viewController: UIViewController?) {
        if let viewControllerr = viewController {
            viewControllerr.view.layer.removeAllAnimations()
            viewControllerr.willMove(toParent: nil)
            viewControllerr.view.removeFromSuperview()
            viewControllerr.removeFromParent()
        }
    }
